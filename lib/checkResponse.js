...
const checkResponse = (response, context) => {
  ...
  if (response.id) {
    logger('info', ['checkResponse', 'successfull'])
    return true
  } else if (response.stack) {
    logger('error', ['checkResponse', 'stack found', response.stack])
    return false
  } else {
    ...
    logger('warn', ['checkResponse', response.message, 'preapproved ?', allowedErrorCode || allowedMessage])
